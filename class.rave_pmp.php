            ...
            class PMProGateway_Rave extends PMProGateway
            {
                ...
                static function pmpro_payment_option_fields($values, $gateway) 
                {
					...
					<tr class="gateway gateway_rave" <?php if ($gateway != "rave") { ?>style="display: none;"<?php 
						...
						<th scope="row" valign="top">	
							<label for="rave_payment_method">
							    <?php _e('Payment Method', 'paid-memberships-pro'); ?>:
							            </label>
						</th>
						<td>
							<select id="rave_payment_method" name="rave_payment_method">
								...
								<option value="all">
								    <?php _e('All', 'paid-memberships-pro'); ?>
								        </option>
								
								<option value="ussd">
								    <?php _e('USSD only', 'paid-memberships-pro'); ?>
								        </option>
								
								<option value="account">
								    <?php _e('Account only', 'paid-memberships-pro'); ?>
								        </option>
								
								<option value="card">
								    <?php _e('Card only', 'paid-memberships-pro'); ?>
								        </option>
								
								 <?php
								    if (!empty($values['rave_payment_method'])) {
								            $selected = $values['rave_payment_method']
								            ;"selected";  } ?>
					...
					<tr class="gateway gateway_rave" <?php if ($gateway != "rave") { ?>style="display: none;"<?php 
						...
						<th scope="row" valign="top">	
							<label for="rave_merchant_country">
							    <?php _e('Merchant Country', 'paid-memberships-pro'); ?>:
							        </label>
						</th>
						<td>
							<select id="rave_merchant_country" name="rave_merchant_country">
								...
								<option value="NG">
								    <?php _e('Nigeria', 'paid-memberships-pro'); ?>
								            </option>
                                                                        
								<option value="GH">
								    <?php _e('Ghana', 'paid-memberships-pro'); ?>
								            </option>
                                                                        
								<option value="KE">
								    <?php _e('Kenya', 'paid-memberships-pro'); ?>
								            </option>
								
								 <?php
								    if (!empty($values['rave_merchant_country'])) {
								            $selected = $values['rave_merchant_country']
								            ;"selected";  } ?>
							</select>
						</td>
					</tr>
                    ...
                    if (pmpro_isLevelRecurring($order->membership_level))
                {
                  ...
                  $url = $baseUrl . 'https://api.ravepay.co/v2/gpx/paymentplans/query?seckey=' . $secretKey . '&q=' . $order->membership_level->name . ' - ' . $amount . ' - ' . $interval;
                  ...
                  if ($resp->data->paymentplans[0]->id < 1) {
                    $url = $baseUrl . 'https://api.ravepay.co/v2/gpx/paymentplans/create';
